...
def get_activity_logs():
    ...
    if filter_type == "user":
        logs_query = ActivityLog.query.filter_by(user_id=user.id)
    else:
        logs_query = ActivityLog.query

    logs = logs_query.order_by(ActivityLog.date.desc()).paginate(page=page, per_page=per_page, error_out=False)
